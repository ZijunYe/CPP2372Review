...
class Point2D {
 public:
    ...
    bool operator<(const Point2D &pt) const { //overload operator "<"
        if (x == pt.x)
            return y < pt.y;
        else
            return x < pt.x;
    ...
    inline std::ostream&
operator<<(std::ostream &left, const Point2D &pt) { //overload output operator 
...
int main()
{
   ...
   //COPY ALL THE THINGS FROM THE FILE 
   std::ifstream is("2-firstname.txt"); //input file 

   std::istream_iterator<std::string> ii(is); //ii = is iterator, ii pointing to the beginning of the file

   std::istream_iterator<std::string> eof; // empty input stream iterator, null(end of the file)

   //create a list 
   std::list<std::string> names(ii, eof);  //copy the file from beginning to the end 
